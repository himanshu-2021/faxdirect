...
class UserController extends Controller
{
    ...
    public function userRegisterPost(Request $request)
    {
        ...
        $v = array('name' => $data['name'], 'email' => $data['email'], 'bussiness_name' => $data['bussiness_name'], 'password' => Hash::make($data['password']), 'role' => 2,'created_at'=>$date ,'updated_at'=>$date);
        if (!empty($data)) {
            try {
            ...
            //   $insertData = array_merge($v , ["database_name"=>$subdomain_db,'subdomain'=>$subdomain]);
                try{
                    $id=DB::table('users')->insertGetId($v);
                }catch(Exception $e){
                    dd($e);
                }
                $request->session()->flash('success', 'Registration successful! Please login to continue');
            //    $connection = DB::reconnect('mysql2');
            //    $id=$connection->table('users')->insertGetId($insertData); //also insert the id of current company
    ...
    public function userLoginPost(Request $request)
    {
        ...
        if(auth()->attempt(array('email' => $input['email'], 'password' => $input['password'])))
        {
            if (auth()->user()->role == 2) {
                ...
                return redirect()->route('web.home')->with('error','you dont have to access!');
            }
        }else{
            return redirect()->route('login')
                ->with('error','Credencials Not Matched.');
        }
    }
